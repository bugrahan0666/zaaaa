...
client.on("ready", () => {
      client.user.setPresence({ activity: { name: `Ses Sistemi Yaptırmak İçin Déf#0004` , type: "PLAYING"}, status: 'idle' })
...
client.on("message", msg => {
    ...
    if (!prefix || prefix && !prefix.length) return console.error(`KOMUT KULLANABİLMEK İÇİN BİR PREFİX GİRMENİZ `)

    if (msg.content.startsWith(prefix)) {
        if (msg.content.toLowerCase() === prefix + "toplantı") {
            if (config.bot.owners.some(r => r == msg.author.id) || msg.author.id == msg.guild.ownerID) {
